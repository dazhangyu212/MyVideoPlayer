...
public class MultipurposeImageView extends AppCompatImageView {
    ...
    public MultipurposeImageView(Context context) {
        this(context, null);
    }

    public MultipurposeImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MultipurposeImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        centerX = x+((right+left)/2);
        centerY = y+((bottom+top)/2);
    ...
    private void initImagePositionAndSize() {
        ...
        if (scaleFactor < 1){
            ...
            //将图片中心点平移到和控件中心点重合
//            mCurrentMatrix.postTranslate(centerX-(mBoundRectF.centerX()*(1-scaleFactor)), centerY - (mBoundRectF.centerY()*(1-scaleFactor)));
            mCurrentMatrix.postTranslate(centerX-(mBoundRectF.centerX()*1.1f), centerY-(mBoundRectF.centerY()*1.1f));
...
//            }
    ...
    protected void translation(float dx, float dy) {
        //检查图片边界的平移是否超过控件的边界
        if (mBoundRectF.left + dx > centerX || mBoundRectF.right + dx < centerX
                || mBoundRectF.top + dy > centerY || mBoundRectF.bottom + dy < centerY) {
//            backTranslation();
            return;
        }
    ...
    protected void backTranslation() {
        ...
        if (mBoundRectF.width() >= getWidth()) {
            ...
            dx = centerX - mBoundRectF.centerX();
        }
        ...
        if (mBoundRectF.height() >= getHeight()) {
            ...
            dy = centerY - mBoundRectF.centerY();
        }
        mCurrentMatrix.postTranslate(dx, dy);
    }
